...
object CaseTeamAPI {
  ...
  case class TeamFormat(users: Seq[CaseTeamUserFormat], groups: Seq[GroupFormat], tenantRoles: Seq[TenantRoleFormat]) {
    PayloadValidator.runDuplicatesDetector("Case team", "user", users.map(_.userId))
    PayloadValidator.runDuplicatesDetector("Case team", "group", groups.map(_.groupId))
    PayloadValidator.runDuplicatesDetector("Case team", "tenant role", tenantRoles.map(_.tenantRole))
  ...
  case class CaseTeamUserFormat(
    ...
    PayloadValidator.required(userId, "Case team users must have a userId")
  ...
  case class TenantRoleFormat(
                               ...
                               caseRoles: Seq[String] = Seq()) {
    PayloadValidator.required(tenantRole, "Tenant role members must have a valid role name")
  ...
  case class GroupFormat(
    ...
    PayloadValidator.required(groupId, "Case team groups must have a groupId")
