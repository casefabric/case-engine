    ...
    caseInstanceSubRoute { (user, caseInstanceId) => {
      path("caseteam") {
        entity(as[Compatible.TeamFormat]) { input =>
          ...
          authorizeCaseAccess(user, caseInstanceId, caseMember => validateTeam(teamInput, caseMember.tenant, team => askModelActor(new SetCaseTeam(caseMember, caseMember.caseIdentifier, team))))
        }
      }
    }
    }
        ...
        entity(as[CaseTeamUserFormat]) { input =>
          authorizeCaseAccess(user, caseInstanceId, {
            caseMember => {
              ...
              onComplete(getUserOrigin(newTeamMember, caseMember.tenant)) {
                case Success(enrichedUser: CaseTeamUser) => askModelActor(new SetCaseTeamUser(caseMember, caseMember.caseIdentifier, enrichedUser))
  ...
  def deleteUser: Route = delete {
    caseInstanceSubRoute { (user, caseInstanceId) =>
      path("caseteam" / "users" / Segment) { userId =>
        askCase(user, caseInstanceId, caseMember => new RemoveCaseTeamUser(caseMember, caseMember.caseIdentifier, userId))
      }
    }
  }
      ...
      path("caseteam" / "groups") {
        entity(as[GroupFormat]) { input =>
          onComplete(validateConsentGroups(Seq(input.asGroup))) {
            case Success(groups: Seq[CaseTeamGroup]) => askCase(user, caseInstanceId, caseMember => new SetCaseTeamGroup(caseMember, caseMember.caseIdentifier, groups.head))
            case Failure(t: Throwable) => complete(StatusCodes.NotFound, t.getLocalizedMessage)
          }
        }
      }
  ...
  def deleteGroup: Route = delete {
    caseInstanceSubRoute { (user, caseInstanceId) =>
      path("caseteam" / "groups" / Segment) { groupId =>
        askCase(user, caseInstanceId, caseMember => new RemoveCaseTeamGroup(caseMember, caseMember.caseIdentifier, groupId))
      }
    }
  }
    ...
    caseInstanceSubRoute { (user, caseInstanceId) => {
      path("caseteam" / "tenant-roles") {
        entity(as[TenantRoleFormat]) { input =>
          askCase(user, caseInstanceId, caseMember => new SetCaseTeamTenantRole(caseMember, caseMember.caseIdentifier, input.asTenantRole))
        }
      }
    }
    }
  ...
  def deleteTenantRole: Route = delete {
    caseInstanceSubRoute { (user, caseInstanceId) =>
      path("caseteam" / "tenant-roles" / Segment) { tenantRoleName =>
        askCase(user, caseInstanceId, caseMember => new RemoveCaseTeamTenantRole(caseMember, caseMember.caseIdentifier, tenantRoleName))
      }
    }
  }
}
