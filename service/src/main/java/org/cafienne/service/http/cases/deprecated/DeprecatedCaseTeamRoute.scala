...
import org.cafienne.model.cmmn.actorapi.command.team.removemember.{RemoveCaseTeamTenantRole, RemoveCaseTeamUser}
    ...
    caseInstanceSubRoute { (user, caseInstanceId) => {
      path("caseteam") {
        entity(as[BackwardCompatibleTeamMemberFormat]) { input =>
          askCase(user, caseInstanceId, caseMember => new DeprecatedUpsert(caseMember, caseMember.caseIdentifier, input.upsertMemberData))
        }
      }
    }
    }
      ...
      path("caseteam" / Segment) { memberId =>
        parameters("type".?) { memberType =>
          if (memberType.nonEmpty && memberType.get == "role") {
            askCase(user, caseInstanceId, caseMember => new RemoveCaseTeamTenantRole(caseMember, caseMember.caseIdentifier, memberId))
          } else {
            askCase(user, caseInstanceId, caseMember => new RemoveCaseTeamUser(caseMember, caseMember.caseIdentifier, memberId))
          }
        }
      }
    }
