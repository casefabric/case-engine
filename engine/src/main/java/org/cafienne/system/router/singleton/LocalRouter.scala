...
package org.cafienne.system.router.singleton
...
class LocalRouter(caseSystem: CaseSystem, actors: mutable.Map[String, ActorRef], terminationRequests: mutable.Map[String, ActorRef]) extends Actor with LazyLogging {
  ...
  private def handleDeserializationFailure(value: DeserializationFailure): Unit = {
  ...
  private def handleUnknownMessage(value: Any): Unit = {
  ...
  /**
    ...
    * @param m
    */
  private def forwardMessage(m: ModelCommand): Unit = {
