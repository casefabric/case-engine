...
class JDBCTimerStore(val dbConfig: DatabaseConfig[JdbcProfile], val tablePrefix: String)
  ...
  override def storeTimer(job: Timer, offset: Option[Offset]): Future[Done] = {
    ...
    val record = TimerServiceRecord(timerId = job.timerId, caseInstanceId = job.caseInstanceId, metadata = job.metadata, moment = job.moment, tenant = "", user = job.userId)
  ...
  override def importTimers(list: Seq[Timer]): Unit = {
    val tx = list
      .map(job => TimerServiceRecord(timerId = job.timerId, caseInstanceId = job.caseInstanceId, metadata = job.metadata, moment = job.moment, tenant = "", user = job.userId))
