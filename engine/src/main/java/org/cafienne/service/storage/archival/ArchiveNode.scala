...
import org.cafienne.service.storage.archival.event.{ArchiveCreated, ArchiveReceived}

case class ArchiveNode(user: StorageUser, metadata: ActorMetadata, actor: RootArchiver) extends OffspringNode with LazyLogging {
  override def createStorageCommand: Any = ArchiveActorData(user, metadata)
  ...
  override protected def uponReceiveEvent(event: StorageEvent): Unit = event match {
    case event: ArchiveCreated => informActor(new ArchiveReceived(user, event.metadata))
