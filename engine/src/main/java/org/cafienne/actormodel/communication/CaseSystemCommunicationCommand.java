...
public abstract class CaseSystemCommunicationCommand extends BaseModelCommand<ModelActor, UserIdentity> implements CaseSystemCommunicationMessage {
    ...
    protected CaseSystemCommunicationCommand(ModelActor sender, ActorMetadata target, ModelCommand command) {
        super(command.getUser(), target);
        this.source = sender.metadata;
        this.command = this.deserializedModelCommand = command;
//        System.out.println(" sending " + getClass().getSimpleName() + "[" + command.getDescription() + "] from " + source + " to " + target.path());
    }
